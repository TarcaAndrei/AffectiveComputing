...
class TransformerTrainer(pl.LightningModule):
    def __init__(self,  text_encoder='distilbert-base-uncased'):
        ...
        self.classifier = Linear(768, 26)
        # self.batch_norm_1 = BatchNorm1d(768)
        # self.downsampling = Linear(768, 384)


    def forward(self, text, img_1, img_2):
        output_img_1 = self.encoder_vit_image_1.forward_features(img_2)[:, 0, :]
        full_output = self.classifier(output_img_1)
        return full_output
    ...
    def validation_step(self, batch, batch_idx):
        ...
        loss = F.cross_entropy(outputs, labels.float(), weight=weights)
...
if __name__ == "__main__":
    ...
    dataloader_train = DataLoader(dataset=train_dataset, batch_size=128, shuffle=True, collate_fn=CustomCollateFn(device=device), drop_last=True)
    dataloader_val = DataLoader(dataset=val_dataset, batch_size=64, collate_fn=CustomCollateFn(device=device), drop_last=True)
