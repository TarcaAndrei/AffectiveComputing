...
class Attention(nn.Module):
    ...
    def get_q_k_v(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        B, N, C = x.shape
        qkv = self.qkv(x).reshape(B, N, 3, self.num_heads, self.head_dim).permute(2, 0, 3, 1, 4)
        q, k, v = qkv.unbind(0)
        q, k = self.q_norm(q), self.k_norm(k)
...
class CrossAttention(nn.Module):
    ...
    def forward_two(self, x_q, x_kv):
        B, N_q, C = x_q.shape
        N_kv = x_kv.shape[1]

        # Query from x_q
        q = self.wq(x_q).reshape(B, N_q, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)  # B N_q C -> B H N_q (C/H)

        # Keys and values from x_kv
        k = self.wk(x_kv).reshape(B, N_kv, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)  # B N_kv C -> B H N_kv (C/H)
        v = self.wv(x_kv).reshape(B, N_kv, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)  # B N_kv C -> B H N_kv (C/H)

        # Attention
        attn = (q @ k.transpose(-2, -1)) * self.scale  # BH N_q N_kv
        attn = attn.softmax(dim=-1)
        attn = self.attn_drop(attn)

        # Weighted sum of values
        x = (attn @ v).transpose(1, 2).reshape(B, N_q, C)  # BH N_q (C/H) -> B N_q C
        x = self.proj(x)
        x = self.proj_drop(x)
        return x
...
class VisionTransformer(nn.Module):
    ...
    def forward_q_k_v(self, x: torch.Tensor) -> torch.Tensor:
        x = self.patch_embed(x)
        x = self._pos_embed(x)
        x = self.patch_drop(x)
        x = self.norm_pre(x)
        if self.grad_checkpointing and not torch.jit.is_scripting():
            ...
            q, k, v = self.first_block.get_q_k_v(x)
